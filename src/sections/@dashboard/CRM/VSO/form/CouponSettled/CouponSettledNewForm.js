...
CouponSettledNewForm.propTypes = {
  doctor_id: PropTypes.string,
};

function CouponSettledNewForm({ doctor_id }) {
  ...
  const handleChangeQuantity = useCallback(
    (event, index) => {
      ...
      setValue(`copons_settled[${index}].quantity`, Number(event.target.value));
      setValue(`copons_settled[${index}].point`, totalPoint);
    },
  ...
  const handleChangeSampleName = useCallback(
    (event, index) => {
      const totalPoint = Number(pointMedicine);
      setValue(`copons_settled[${index}].copon_name`, event.target.value);
      setMedicineName(event.target.value);
      setValue(`copons_settled[${index}].doctor_id`, doctor_id);
      setValue(`copons_settled[${index}].point`, totalPoint);
      trigger(`copons_settled[${index}].copon_name`);
      // resetField(`copons_settled[${index}].quantity`);
    },
          ...
          {fields?.map((item, index) => (
            <Stack key={item.id} alignItems="flex-end" spacing={1.5}>
              <Stack direction={{ xs: 'column', md: 'row' }} spacing={2} sx={{ width: 1 }}>
                <RHFSelect
                  name={`copons_settled[${index}].copon_name`}
                  label="Medicine Name"
                ...
                <RHFTextField
                  size="small"
                  type="number"
                  name={`copons_settled[${index}].quantity`}
                  label="No. of Coupon Settled"
                  placeholder="0"
                ...
                <RHFTextField
                  size="small"
                  type="number"
                  name={`copons_settled[${index}].point`}
...
export default CouponSettledNewForm;
