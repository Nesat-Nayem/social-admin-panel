...
export default function SampleForm({ isEdit = false, currentUser, save, MEDICINE_OPTIONS, v_id }) {
  ...
  const handleChangeVsoId = useCallback(
    (event, index) => {
      ...
      setValue(`samples[${index}].vsoid`, event.target.value);
    },
    [setValue]
  );
  ...
  const handleChangeSampleName = useCallback(
    (event, index) => {
      setValue(`samples[${index}].samplename`, event.target.value);
      setValue(`samples[${index}].vsoid`, v_id);
    },
    [setValue, v_id]
  );
            ...
            >
              ...
              >
                {MEDICINE_OPTIONS.map((items) => (
                  <MenuItem key={items.value} value={items.label}>
                    {items.label}
                  </MenuItem>
                ))}
              </RHFSelect>
