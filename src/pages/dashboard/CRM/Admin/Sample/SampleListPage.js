...
export default function SampleListPage() {
  ...
  const { data, isLoading } = useGetMaster('sample-name');
  const { deleteUser, isSuccess } = useDeleteMaster('sample-name');
  ...
  const _sampleList =
    data?.map((sample) => ({
      ...
      name: sample?.name,
      point: sample?.point,
      created_at: moment(sample.created_at).format('DD-MM-YYYY'),
      updated_at: moment(sample.updated_at).format('DD-MM-YYYY'),
    })) || [];
  ...
  const handleEditRow = (id) => {
    ...
    navigate(PATH_DASHBOARD.samplemaster.edit(id));
  };
        ...
        <CustomBreadcrumbs
          ...
          action={
            <Button
              ...
              to={PATH_DASHBOARD.samplemaster.new}
              variant="contained"
