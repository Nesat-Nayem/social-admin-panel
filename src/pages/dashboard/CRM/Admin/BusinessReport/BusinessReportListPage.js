...
import {
  UserTableRow,
  UserTableToolbar,
} from '../../../../../sections/@dashboard/CRM/Admin/BusinessReport/list';
...
const _businessList = [...Array(24)].map((_, index) => ({
  id: _mock.id(index),
...
export default function BusinessReportListPage() {
  ...
  const todayFormated = moment(new Date()).format('YYYY-MM-DD');

  const [fromDate, setFromDate] = useState(todayFormated);
  const [toDate, setToDate] = useState(todayFormated);

  console.log('LLL', fromDate, toDate);
  ...
  const handleEditRow = (id) => {
    console.log('jjjEdit');
    navigate(PATH_DASHBOARD.incentivemaster.edit(paramCase(id)));
  };
  ...
  return (
    <>
      <Helmet>
        <title> Business Report: List | Voiz</title>
      </Helmet>

      <Container maxWidth={themeStretch ? false : 'lg'}>
        <CustomBreadcrumbs
          heading="Business Report List"
          links={[
          ...
          //     to={PATH_DASHBOARD.incentivemaster.new}
          //     variant="contained"
