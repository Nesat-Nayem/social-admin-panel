...
import {
  UserTableRow,
  UserTableToolbar,
} from '../../../../../sections/@dashboard/CRM/Admin/Vehicle/list';
...
const TABLE_HEAD = [
  ...
  { id: 'category', label: 'Vehicle Type', align: 'left' },
  { id: 'allowance', label: 'Rs/Km', align: 'left' },
...
export default function VehicleListPage() {
  ...
  const { data, isLoading } = useGetMaster('car-allowance');
  const { deleteUser, isSuccess } = useDeleteMaster('car-allowance');
  ...
  const handleEditRow = (name) => {
    console.log('jjjEdit', name);
    // navigate(PATH_DASHBOARD.vehiclemaster.edit(paramCase(id)));
    navigate(PATH_DASHBOARD.vehiclemaster.edit(name));
  };
        ...
        <CustomBreadcrumbs
          ...
          action={
            <Button
              ...
              to={PATH_DASHBOARD.vehiclemaster.new}
              variant="contained"
...
function applyFilter({ inputData, comparator, filterName, filterStatus, filterRole }) {
  ...
  if (filterName) {
    inputData = inputData.filter(
      (user) => user.category.toLowerCase().indexOf(filterName.toLowerCase()) !== -1
    );
  }
